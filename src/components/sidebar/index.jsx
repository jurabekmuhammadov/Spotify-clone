...
const Sidebar = () => {
    ...
    return (
        <aside className="h-screen w-full bg-black p-2 flex flex-col gap-2">
            <div className={`top bg-zinc-900 rounded-lg p-4 ${expanded ? "w-full" : "w-1/4 py-4 px-0"}`}>
                <ul className="flex flex-col gap-5">
                    <li className="w-full menu-item">
                        <NavLink to={"/"} className={`w-full flex items-center gap-4 text-zinc-500 transition-all ${expanded ? "" : "justify-center"}`}>
                            ...
                            <span className={`font-semibold hover:text-white ${expanded ? "" : "hidden"}`}>Home</span>
                        </NavLink>
                    </li>
                    <li className="w-full menu-item">
                        <NavLink to={"/search"} className={`w-full flex items-center gap-4 text-zinc-500 transition-all ${expanded ? "" : "justify-center"}`}>
                            ...
                            <span className={`font-semibold hover:text-white ${expanded ? "" : "hidden"}`}>Search</span>
                        </NavLink>
                    </li>
                    <li className="w-full menu-item">
                        <NavLink to={"/library"} className={`w-full flex items-center gap-4 text-zinc-500 transition-all ${expanded ? "" : "justify-center"}`}>
                            <svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className={`font-semibold hover:text-white ${expanded ? "" : "hidden"}`}>Your Library</span>

                        </NavLink>
                    </li>
                    <li className="w-full menu-item">
                        <NavLink to={"/create-playlist"} className={`w-full flex items-center gap-4 text-zinc-500 transition-all ${expanded ? "" : "justify-center"}`}>
                            <svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd" clipRule="evenodd" d="M2 0C0.895431 0 0 0.895431 0 2V30C0 31.1046 0.895431 32 2 32H30C31.1046 32 32 31.1046 32 30V2C32 0.895431 31.1046 0 30 0H2ZM15 9H17V15H23V17H17V23H15V17H9V15H15V9Z" fill="rgb(113, 113, 113)" />
                            </svg>
                            <span className={`font-semibold hover:text-white ${expanded ? "" : "hidden"}`}>Create playlist</span>
                        </NavLink>
                    </li>
                    <li className="w-full menu-item">
                        <NavLink to={"/liked-songs"} className={`w-full flex items-center gap-4 text-zinc-500 transition-all ${expanded ? "" : "justify-center"}`}>
                            <svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className={`font-semibold hover:text-white ${expanded ? "" : "hidden"}`}>Liked Songs</span>
                        </NavLink>
                    </li>
                </ul>
            </div>
            <div className={`bottom bg-zinc-900 rounded-lg p-2 pt-3 overflow-auto ${expanded ? "w-full" : "w-1/4 py-4 px-0"} h-screen flex items-end justify-start flex-col`}>
                <button onClick={() => setExpanded((curr) => !curr)} className={`w-full flex justify-end ${expanded ? "" : "justify-center"}`}>
                    {expanded ? <PanelRightOpen color="gray" size={26} /> : <PanelLeftOpen color="gray" size={26} />}
                </button>
                <ul className="flex flex-col w-full mt-3">
                    {featuredPlaylist.map((item) => (
                        <div onClick={() => goPlaylist(item, item.id)} to={"/playlist"} className={`text-white flex items-center gap-4 capitalize font-medium p-2 cursor-pointer rounded-md hover:bg-zinc-800 ${expanded ? "" : "flex items-center justify-center"}`} key={item.id}>
                            <img src={item.images[0].url} alt="img" className="w-12 h-12 rounded-md" />
                            {expanded ? item.name : ""}
                        </div>
                    ))}
                </ul>
